...
module end(opening, internal, height, width){
  translate([0,(width/2)+(opening/2),height/2])
    rotate([90,0,90]){
    hull(){
...
module rail(length, height, base_width, top_width, top_extend) {
  ...
  translate([length,0,0]){
    end(insert_opening-0.5,insert_internal-0.5,height, top_width);
  }

  difference() {
    bar(length, height, base_width, top_width, top_extend);
    end(insert_opening,insert_internal,height, top_width);
  }
}
